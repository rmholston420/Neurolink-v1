...
class TestComputeFocusScore:
    def test_high_beta_low_theta_gives_high_focus(self):
        score = compute_focus_score(alpha=0.3, beta=0.6, baseline_alpha=0.3)
        assert score > 0.5

    def test_high_theta_low_beta_gives_low_focus(self):
        score = compute_focus_score(alpha=0.1, beta=0.05, baseline_alpha=0.3)
        assert score < 0.5

    def test_returns_float(self):
        score = compute_focus_score(alpha=0.3, beta=0.3, baseline_alpha=0.3)
        assert isinstance(score, float)

    def test_score_in_unit_interval(self):
        for alpha in [0.0, 0.2, 0.5, 1.0]:
            for beta in [0.0, 0.2, 0.5, 1.0]:
                score = compute_focus_score(alpha=alpha, beta=beta, baseline_alpha=0.3)
                assert 0.0 <= score <= 1.0, f"score={score} out of [0,1] for alpha={alpha} beta={beta}"


class TestClassifyFocus:
    def test_high_score_returns_focused(self):
        state = classify_focus(0.85)
        assert "focus" in state.value.lower() or state.value != "unknown"
...
class TestFatigueDetector:
    def test_initial_score_is_zero(self):
        fd = FatigueDetector()
        # Before any update, internal state should be at baseline
    ...
    def test_high_theta_low_alpha_increases_fatigue(self):
        ...
        # Fatigued trajectory should end higher than alert trajectory
    ...
    def test_reset_clears_accumulator(self):
        ...
        # Score should be low immediately after reset (EMA starts fresh)
