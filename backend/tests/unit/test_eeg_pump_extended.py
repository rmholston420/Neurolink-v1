...
_FAKE_PPG = PPGPayload(hr_bpm=60.0, ibi_ms=[833.0], sdnn_ms=20.0, rmssd_ms=18.0)
...
def _make_eeg_sample(
    ...
    # in eeg_pump._build_payload.  Shorter buffers correctly yield ppg=None
    # at runtime (warmup), but tests exercising the PPG path need a full buffer.
...
class TestTickStage0:
    ...
    async def test_tick_calls_gate_sample_when_imu_toggle_on(self):
        ...
        toggles = FilterToggleConfig(**{**_all_toggles_off().__dict__, "imu_gate": True})
    ...
    async def test_tick_emits_settling_when_not_acquisition_ready(self):
        ...
        with patch("neurolink.eeg_pump.filter_toggles") as ft, \
             patch("neurolink.eeg_pump.impedance") as imp:
            ft.get_toggles.return_value = _all_toggles_off()
            imp.check.return_value = True
            await pump._tick()
...
class TestBuildPayloadDSPPaths:
    ...
    async def test_stage1_fir_applied_when_toggle_on(self):
        ...
        toggles = FilterToggleConfig(**{**_all_toggles_off().__dict__, "stage1_fir": True})
    ...
    async def test_ppg_buffer_produces_ppg_payload(self):
        ...
        compute_ppg is a local import inside _build_payload, so the correct
        patch target is the source module 'neurolink.dsp.ppg.compute_ppg'.
        Patching the source redirects every caller regardless of import site.
        """
        sample = _make_eeg_sample(ppg=True)
        pump = _make_pump()

        with patch("neurolink.eeg_pump.filter_toggles") as ft, \
             patch("neurolink.dsp.ppg.compute_ppg", return_value=_FAKE_PPG):
            ft.get_toggles.return_value = _all_toggles_off()
            payload = await pump._build_payload(sample)
