...
const styles: Record<string, React.CSSProperties> = {
  ...
  indicator: (poor: boolean): React.CSSProperties => ({
    display: 'inline-flex',
    alignItems: 'center',
    gap: 8,
    padding: '8px 16px',
  ...
  dot: (poor: boolean): React.CSSProperties => ({
    width: 10,
    height: 10,
    borderRadius: '50%',
...
export default function ContactQuality({ poorContact, contactQuality }: Props) {
  ...
  return (
    <div style={styles.container}>
      <span style={styles.indicator(poorContact)}>
        <span style={styles.dot(poorContact)} />
